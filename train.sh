...
pretrained_model="/mnt/data/illustriousXL_v01.safetensors" # base model path | 底模路径
model_type="sdxl"                        # option: sd1.5 sd2.0 sdxl flux | 可选 sd1.5 sd2.0 sdxl flux。SD2.0模型下 clip_skip 默认无效
parameterization=0                        # parameterization | 参数化 本参数需要在 model_type 为 sd2.0 时才可启用

train_data_dir="../dataset" # train dataset path | 训练数据集路径
